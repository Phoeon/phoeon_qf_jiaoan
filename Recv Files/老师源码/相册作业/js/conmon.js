...
//时间运动框架(有待完善)
function moveTime( obj , json , time , cv , fn ){
	//t
	...
	var iC = {};
	for(var attr in json){
		iC = parseInt( json[attr] ) - iB[attr];
	}
	obj.timer = setInterval(function(){
		var nowTime = ( new Date() ).getTime();
		var iT = Math.min( nowTime - firstTime , time);
		if(iT == time){
			clearInterval(obj.timer);
			if(fn){
				fn();
		...
		for(var attr in json){
			var value = Tween[cv]( iT , iB[attr] , parseInt( json[attr] ) - iB[attr] , time);
